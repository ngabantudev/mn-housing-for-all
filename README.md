...
Three layers, one question each. All three can be on at once — the point is reading them against each other.

- **Vacant buildings** — 695 buildings the Twin Cities currently carry on their vacant-building registers, and how long each has sat that way. Saint Paul records what each one is (297 homes, 73 businesses, 14 mixed-use); Minneapolis publishes no building-type field, so its 311 records are shown as unclassified rather than assumed to be houses. Click one for its registration date, ward, the conditions that put it on the register, and (in Saint Paul) the category controlling whether it can legally be sold or reoccupied.
- **Tax subsidy districts** — Saint Paul's 64 tax-increment financing districts, with the tax increment each has received as filed with the State Auditor. 29 are Housing districts, which Minnesota conditions on producing low-income housing. Brought to this project by Robert Hindel of [Fairtax St Paul](https://www.fairtaxstpaul.org/).
- **Cooling sites** — 192 indoor, publicly accessible locations from Hennepin County's Cooling Option Map, 162 of them free to enter.

Every source is listed with a description and a link on the [Open Sources](https://mn-housing-for-all.resistance.workers.dev/sources) page — 27 of them, covering the feeds the map pulls from, the cities' own portals, the reporting, the research, the budget documents, and the statutes. If a number appears anywhere in the app, the thing it came from is on that page, along with any limitation its publisher states.
...
npm run test:e2e          # or: npm run test:e2e -- https://your-preview-url
```

`tests/map.mjs` drives a real browser at six viewport sizes. It checks the two things that look fine in a diff and are broken on screen: that the detail card and the filter panel never overlap (measured as rectangles, not eyeballed), and that every filter closes an open detail card when it hides the feature that card describes — and leaves it alone when it doesn't.
...
npm run data:relief       # Hennepin County cooling sites
```
...
The Cooling sites layer is also not a shelter-bed layer, and the app never calls it one — no Minnesota agency publishes an open, current list of shelter locations and bed counts. See the sources page for the full note on that gap.
